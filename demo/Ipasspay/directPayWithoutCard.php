...
use Ipasspay\IpasspayChannel\service\IpasspayService;
    ...
    //No card information is connected to ipasspay's payment gateway (such as digital currency payment gateway, the actual processing logic is basically the same as direct connection, if successful, there will be a payment link in the return result for further access by users).
    //Compared to the direct connection interface, second parameter "false"(indicating whether credit card information is taken) is added when $ipasspay_service->onlinePay($request_data,false) is called.
    ...
    if (!$ipasspay_service->setConfig($config)->onlinePay($request_data,false)) {*/
    if (!$ipasspay_service->onlinePay($request_data,false)) {
